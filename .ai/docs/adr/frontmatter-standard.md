...
- **Purpose:** Categorize document purpose for tooling/filtering
- **Standard Values:**
  - `overview` - High-level summary or index document
  - `phase` - Implementation phase document
  - `checklist` - Verification or tracking checklist
  - `reference` - Reference material or specification
...
- **Format:** `YYYY-MM-DD`
- **Purpose:** Original creation date (distinct from `date` which may be last update)
...
These fields are used specifically in plan and phase documents.

#### `depends_on` (Array of UUIDs)

- **Format:** YAML array of document UUIDs
- **Purpose:** Define phase execution order / prerequisites
- **Relationship:** Directional (this phase depends on listed phases)
- **Example:**
  ```yaml
  depends_on:
    - 6daf1a38-745e-41f8-abf4-90757b4b1a8a  # phase-00
    - 367b75a1-dd7d-4d34-9bf9-6aaab692c2ee  # phase-01
  ```
- **Usage:** "This phase cannot start until these phases are complete"

#### `checklists` (Array of UUIDs)

- **Format:** YAML array of checklist document UUIDs
- **Purpose:** Link phase to associated verification checklists
- **Example:**
  ```yaml
  checklists:
  ...
  ```
- **Usage:** "Use these checklists to verify phase completion"

#### `references` (Array of Strings)

- **Format:** YAML array of reference identifiers or paths
- **Purpose:** Link to supporting documents (ADRs, specs, external docs)
- **Example:**
  ```yaml
  references:
    - dependency-scanning-strategy-adr
    - wiki-documentation-strategy-adr
    - https://example.com/external-spec
  ```
- **Usage:** "Consult these references for context"

#### `issues` (Array of Strings or Objects)

- **Format:** YAML array of GitHub issue references
- **Purpose:** Track associated GitHub issues for the phase
- **Example:**
  ```yaml
  issues:
    - "#123"
    - "owner/repo#456"
  issues: []  # Empty array if no issues yet
  ```
- **Usage:** "These issues track work for this phase"

#### `gate` (Object)

- **Format:** YAML object with gate configuration
- **Purpose:** Define quality gate requirements before phase can begin
- **Fields:**
  - `required` (Boolean): Whether gate review is mandatory
  - `justfile_changes` (String): Expected justfile impact (`none` | `minor` | `major`)
  - `review_focus` (Array of Strings): Topics to cover in gate review
- **Example:**
  ```yaml
  gate:
    required: true
    justfile_changes: major
    review_focus:
      - Initial justfile structure
      - Configuration options
      - 1Password integration
  ```
- **Usage:** "Complete this gate review before starting the phase"
